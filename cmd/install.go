...
func RunInstall(opts *Options, tempName, tempPath string) error {
	...
	PrintVerbose(2, "Install File: %s", TempInstallFile)
	...
	installType, err := DetectInstallType(TempInstallFile)
	if err != nil {
		return fmt.Errorf("detect install file type: %w", err)
	}

	base := filepath.Base(TempInstallFile)

	var extraArgs string

	switch installType {
	case "preseed":
		extraArgs = fmt.Sprintf("auto=true priority=critical file=/%s", base)
	case "kickstart":
		extraArgs = fmt.Sprintf("inst.ks=file:/%s", base)
	default:
		return fmt.Errorf("could not determine install file type (kickstart vs preseed) for %s", TempInstallFile)
	}
